...
Setting Up Linux Setup for V12 Elasticsearch Upgrade from 7.9 `}),e.add({id:10,href:"/modules/siem/log_analytics/filebeat/",title:"Filebeat",content:`About Filebeat Filebeat is a software that runs on your managed host or emedge. It collects logs and parse them to emedge for aggregation and forwarding to NetGain Systems, where you can analyze the data.
...
`}),e.add({id:15,href:"/modules/netflow/",title:"NetFlow",content:`NetFlow are used to collect and monitor network traffic data.They monitor IP addresses,data volumes,time,ports and protocols.
Using Network Traffic Analytics Reports Watcher Settings and Maintenance `}),e.add({id:16,href:"/installation/emedge/netgain-agent/",title:"NetGain Agent",content:`netgain-agent is a agent software that runs on managed server. It collects metrics and logics from the server and forwards the data to EM or Emedge
...
EMIP=x.x.x.x specifies the IP address of EM or Emedge server SERVICE=1 specifies to automatically install netgain-agent as a system service If you do not wish to install as a system service, set to SERVICE=0 Installation steps on other Operating Systems Download netgain-agent-v12-latest.tar.gz Extract the package cd /opt tar zxf netgain-agent-v12-latest.tar.gz cd netgain Configures the EM destination IP edit agent.properties to add &ldquo;serverHost=x.x.x.x&rdquo; Start the agent cd /opt/netgain/bin &amp;&amp; ./agent_start.sh Additional notes All logs will be saved into agent.log in the installation directory To stop netgain-agent, simply run netgain-agent stop Run netgain-agent -h for help on more commands `}),e.add({id:17,href:"/installation/emedge/emedge-agent/",title:"Emedge Agent",content:`About emedge-agent emedge-agent is a software that runs on your managed hosts. It collects monitoring and performance data from hosts and sends them to emedge for aggregation and forwarding to NetGain Systems, where you can analyze the data.
