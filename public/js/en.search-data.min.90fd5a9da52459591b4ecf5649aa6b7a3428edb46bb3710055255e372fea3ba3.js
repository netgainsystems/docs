...
Left Navigation Top Navigation Dashboards Maps Business Views `}),e.add({id:9,href:"/installation/cloud_vista/",title:"Cloud Vista",content:`Cloud Vista Suite (CVS) by NetGain Systems is a scalable monitoring service for hybrid cloud environments, providing monitoring of networks, servers, databases, applications, and services, through a SaaS-based monitoring analytics platform
...
Setting Up `}),e.add({id:10,href:"/modules/siem/log_analytics/filebeat/",title:"Filebeat",content:`About Filebeat Filebeat is a software that runs on your managed host or emedge. It collects logs and parse them to emedge for aggregation and forwarding to NetGain Systems, where you can analyze the data.
...
Zscaler module
Find steps here for installation of Filebeat: Installation
`}),e.add({id:11,href:"/modules/metrics/",title:"Metrics",content:`The Metrics function within our infrastructure monitoring system gives you a comprehensive overview of the status of your IT devices, enabling proactive management and early detection of potential issues. By monitoring key metrics, you can address problems before they escalate, ensuring smooth operation and minimizing any impact on your end-users.
Discovering / Adding Devices to be Monitored Monitoring by Device Type Monitoring by Alert Type Monitoring by Business Type Editing Monitoring Settings Threshold Settings Metrics Policy Customize SNMP, Scripts, Database and Dynamic Thresholds Metrics Profile `}),e.add({id:12,href:"/installation/elasticsearch_7_9_upgrade/",title:"Elasticsearch Upgrade from 7.9",content:`1. Disable check_box.sh in crontab crontab -e 1#* * * * * /root/netgain/bin/check_box.sh 2#*/15 * * * * /root/netgain/bin/ha/standby.sh 310 0 * * * mon sync; echo 3 &gt; /proc/sys/vm/drop_caches 40 6 * * * find /var/log/iossd* -mtime +30 -delete 2. Stop NetGain EM /root/netgain/bin/box-stop.sh 3. Export Elasticsearch indices for backup a. View indices curl -X GET “&lt;localhost_IP&gt;:9200/_cat/indices” b. Create directory where to store exported indices mkdir /root/exported_indices c. Export each index /root/netgain/bin/run.sh netgain.elastic.IndexExporter index_name /root/exported_indices/index_name 4. Stop Elasticsearch systemctl stop elasticsearch 5. Download Elasticsearch 7.17.0 wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.17.0-x86_64.rpm --no-check-certificate 6. Install Elasticsearch 7.17.0 yum localinstall --disablerepo=* elasticsearch-7.17.0-x86_64.rpm 7. Start Elasticsearch systemctl start elasticsearch 8. Check Elasticsearch status systemctl status elasticsearch 9. Repeat steps 4 to 6 for Elasticsearch 8.11.4 installation Replace version with 8.11.4 in steps 4 to 6.
10. Edit elasticsearch.yml for Elasticsearch 8.11.4 vi /etc/elasticsearch/elasticsearch.yml a. Remove old node roles below: 1node.master: true 2node.data: true 3node.ingest: true b. Add new node roles below: 1node.roles: [ data, master, ingest ] 11. Start Elasticsearch systemctl start elasticsearch 12. Check Elasticsearch status systemctl status elasticsearch `}),e.add({id:13,href:"/installation/emedge/",title:"Emedge",content:`About emedge emedge is a software that runs on your local site. It collects metrics and logics from the IT network and hosts (with emedge-agent installed) and sends them to central monitoring site, where you can analyze and visualize your monitoring and performance data. You only need to deploy an instance of emedge per local site.
...
Install and configure `}),e.add({id:14,href:"/modules/netflow/",title:"NetFlow",content:`NetFlow are used to collect and monitor network traffic data.They monitor IP addresses,data volumes,time,ports and protocols.
Using Network Traffic Analytics Reports Watcher Settings and Maintenance `}),e.add({id:15,href:"/installation/emedge/netgain-agent/",title:"NetGain Agent",content:`netgain-agent is a agent software that runs on managed server. It collects metrics and logics from the server and forwards the data to EM or Emedge
...
EMIP=x.x.x.x specifies the IP address of EM or Emedge server SERVICE=1 specifies to automatically install netgain-agent as a system service If you do not wish to install as a system service, set to SERVICE=0 Installation steps on other Operating Systems Download netgain-agent-v12-latest.tar.gz Extract the package cd /opt tar zxf netgain-agent-v12-latest.tar.gz cd netgain Configures the EM destination IP edit agent.properties to add &ldquo;serverHost=x.x.x.x&rdquo; Start the agent cd /opt/netgain/bin &amp;&amp; ./agent_start.sh Additional notes All logs will be saved into agent.log in the installation directory To stop netgain-agent, simply run netgain-agent stop Run netgain-agent -h for help on more commands `}),e.add({id:16,href:"/installation/em_setup_v12/",title:"Setup NetGain Systems for v12",content:`Table of Contents Requirement and additional packages Installation of ElasticSearch Step 1. Install and Configure JVM Step 2. Configure elasticsearch Step 3. Start and enable elasticsearch service on boot Installation of NetGain EM 1. Download v12 Base software 2. Updated Java and Tomcat 3. Disable firewall 4. Configure rc.local (bold fonts &gt; change it accordingly for customer) 5. Give full permission for rc.local 6. Configure vi /etc/sysconfig/i18n for english language during bootup process 7. Change NetGain application configuration 8. Setup Date and Time 9. Check DNS record 10. Add EM process check to cronjob 11. Run export NG_LICENSE_BY_MACADDR=1 12. Get register key 13. To update the license 14. Update with the Latest patch 15. Export JAVA path 16. Update the Elasticsearch password in config_customize of NetGain 17. Start NetGain EM process Requirement and additional packages Recommended OS: Instal l Rocky Linux 9.2 x86_64 Install additional Packages yum install -y tar pinentry net-snmp net-snmp-utils net-tools telnet openssh openssh-clients ftp expect wget traceroute unzip zip vim openssl Installation of ElasticSearch Step 1. Install and Configure JVM Install Elasticsearch:
...
The generated password for the elastic built-in superuser is : t_uYH54WkAddVJ8qAK9s
...
Reset the password of the elastic built-in superuser with &rsquo;/usr/share/elasticsearch/bin/elasticsearch-reset-password -u elastic'.
Generate an enrollment token for Kibana instances with &rsquo;/usr/share/elasticsearch/bin/elasticsearch-create-enrollment-token -s kibana'.
Generate an enrollment token for Elasticsearch nodes with &rsquo;/usr/share/elasticsearch/bin/elasticsearch-create-enrollment-token -s node'.
...
cluster.name: netgainelkdb # this is to define a cluster name node.name: netgainnode-1 #this is to define a node name network.host: 127.0.0.1 #this is the IP address that the localhost or other server going to contact with elasticsearch. discovery.type: single-node # Enable security features xpack.security.enabled: true xpack.security.enrollment.enabled: false # Enable encryption for HTTP API client connections, such as Kibana, Logstash, and Agents xpack.security.http.ssl: enabled: false keystore.path: certs/http.p12 # Enable encryption and mutual authentication between cluster nodes xpack.security.transport.ssl: enabled: false verification_mode: certificate keystore.path: certs/transport.p12 truststore.path: certs/transport.p12 # Create a new cluster with the current node only # Additional nodes can still join the cluster later #cluster.initial_master_nodes: ["localhost"] # Allow HTTP API connections from anywhere # Connections are encrypted and require user authentication http.host: 0.0.0.0 # Allow other nodes to join the cluster from anywhere # Connections are encrypted and mutually authenticated #transport.host: 0.0.0.0 ** change the details of the underlined bold using the above settings
...
Installation of NetGain EM 1. Download v12 Base software Download link: https://drive.google.com/file/d/16CX1TXhzT7kUhrjhfZyaMrKfmmKVdicX/view?usp=sharing
...
2. Updated Java and Tomcat export PATH=/root/netgain/jre/bin:$PATH
Java version &#34;1.8.0_311&#34; Java(TM) SE Runtime Environment (build 1.8.0_311-b11) Java HotSpot(TM) 64-Bit Server VM (build 25.311-b11, mixed mode) Tomcat version: v9.0.73 3. Disable firewall service firewalld stop
4. Configure rc.local (bold fonts &gt; change it accordingly for customer) vi /etc/rc.local
touch /var/lock/subsys/local systemctl stop firewalld export NG_LICENSE_BY_MACADDR=1 export IOSS_HOME=/root/netgain export NETGAIN_SERIAL=NGSG2209001 export EM_MODEL=EM-BASE export EM_CUSTOMER_NAME=Customer_name su - root -c &#34;$IOSS_HOME/bin/agent_start.sh&#34; systemctl start elasticsearch.service su - root -c &#34;$IOSS_HOME/bin/box.sh&#34; example:
NGSG2209001 should be change to your own serial key
whereas:
NG – NetGain (partner/customer name) SG – Singapore (country) 22 – 2022 (year) 09 – Sept (month) 001 – running number
EM-BASE-Demo
whereas:
Demo - base model of the license
5. Give full permission for rc.local chmod +x /etc/rc.d/rc.local
systemctl enable rc-local
systemctl start rc-local
6. Configure vi /etc/sysconfig/i18n for english language during bootup process vi /etc/sysconfig/i18n
LANG=&#34;en_US.UTF-8&#34; SYSFONT=&#34;latarcyrheb-sun16&#34; SUPPORTED=&#34;en_US.UTF-8:en_US:en&#34; 7. Change NetGain application configuration vi /etc/ng.conf
export SKIP_WDT=1 export CHARSET=en_US export MAX_MEM=1000 #change 50 - 70% of your VM total RAM export NG_LICENSE_BY_MACADDR=1 JAVA_OPTS=&#34;\${JAVA_OPTS} -Delk=1&#34; JAVA_OPTS=&#34;\${JAVA_OPTS} -Delasticsearch_ip=127.0.0.1&#34; 8. Setup Date and Time hwclock --set --date='12/22/2014 14:35'
date -s &quot;22 December 2014 14:35&quot;
yum install chrony
timedatectl set-ntp yes
service chronyd start
(*NTP is another option for auto date and time: ntpdate pool.ntp.org)
9. Check DNS record vi /etc/resolv.conf
nameserver 8.8.8.8 customer DNS should be put after nameserver
10. Add EM process check to cronjob crontab -e
#* * * * * /root/netgain/bin/check_box.sh #*/15 * * * * /root/netgain/bin/ha/standby.sh 10 0 * * * mon sync; echo 3 &gt; /proc/sys/vm/drop_caches 0 6 * * * find /var/log/iossd* -mtime +30 -delete Note: this command is to check if NetGain is inaccessible and it will auto restart the service (remember to remove the # to activate)
11. Run export NG_LICENSE_BY_MACADDR=1 export NG_LICENSE_BY_MACADDR=1
echo $NG_LICENSE_BY_MACADDR
12. Get register key Copy paste below output on notepad, save and Email to NetGain team to request for license
/root/netgain/bin/run.sh netgain.sac.Register
13. To update the license Paste the contents of license from NetGain team to license.txt
vi /root/netgain/bin/license.txt
14. Update with the Latest patch Please ask NetGain support email for the latest patch file.
...
15. Export JAVA path export PATH=/root/netgain/jre/bin:$PATH
And need to add this command globally to /etc/profile and /root/.bash_profile and /root/.bashrc
16. Update the Elasticsearch password in config_customize of NetGain vi /root/netgain/conf/admin/config_customize.xml
&lt;config key=&#34;elasticsearch_password&#34; value=&#34;t_uYH54WkAddVJ8qAK9s&#34;/&gt; 17. Start NetGain EM process /root/netgain/bin/box.sh
...
`}),e.add({id:17,href:"/installation/emedge/emedge-agent/",title:"Emedge Agent",content:`About emedge-agent emedge-agent is a software that runs on your managed hosts. It collects monitoring and performance data from hosts and sends them to emedge for aggregation and forwarding to NetGain Systems, where you can analyze the data.
Installation Configure Checks `}),e.add({id:18,href:"/modules/siem/security_analytics/",title:"Security Analytics",content:`NetGain Systems Security Analytics can manage logs and other data from a wide range of devices including:
...
`}),e.add({id:23,href:"/modules/siem/log_analytics/",title:"Log Analytics",content:`NetGain Systems Log Analytics can manage logs and other data from a wide range of devices including:
...
`}),e.add({id:25,href:"/security/report/",title:"Report",content:"In construction "}),e.add({id:26,href:"/modules/cmdb/",title:"CMDB",content:" Network Config "}),e.add({id:27,href:"/news/Release/",title:"Release notes",content:`NetGain EM Version Release Notes Please find below for the past release notes:
...
Adding Devices to cmdb Device Configuration Management Bulk Operations Configuration Templates Scheduling Configuration Backup Changes `}),e.add({id:30,href:"/posts/",title:"News",content:""}),e.add({id:31,href:"/modules/alerts/",title:"Alerts",content:`This section documents the alerts and notifications options in the software, and the information on settings.
Monitor Alerts Current Alerts History Alerts Alert Policies Alert Categories Alert Correlation Action Triggers Alert Notifications Notification Rules Notification Email Other Alert Notification Settings 3rd Party Notification Integration Integration with 3rd Party Ticketing Apps `}),e.add({id:32,href:"/installation/",title:"Installation",content:`Installations related will be available here.
`}),e.add({id:33,href:"/administrative/sysadmin/",title:"Sys Admin & Tools",content:`This section documents the systems administration functions and the tools provided to help the administrator.
Admin Tools `}),e.add({id:34,href:"/modules/",title:"Modules",content:""}),e.add({id:35,href:"/administrative/",title:"Adminstrative",content:""}),e.add({id:36,href:"/news/",title:"News",content:`Product realted news will be available here.
`}),e.add({id:37,href:"/security/",title:"Security",content:`We care about security. If you have any questions, or encounter any issues, please contact us at security@netgain-systems.com. If you believe you’ve discovered a bug in NetGain Systems product(s) security, please get in touch at security@netgain-systems.com and we will get back to you within 24 hours, and usually earlier. We request that you not publicly disclose the issue until we have had a chance to address it.
...
Selecting Status Click onto the Status tab to reveal the details `}),e.add({id:40,href:"/modules/dashboards_2.0/dashboards2/",title:"Dashboard 2.0",content:`Dashboard 2.0 is your go-to tool for visualizing and tracking key performance indicators (KPIs), metrics, and critical data points. It offers a powerful and intuitive interface to monitor your most important metrics in real-time, helping you make data-driven decisions with confidence.
To explore the full range of features available in Grafana, which powers Dashboard 2.0, click here here to learn more
Home This Home tab provides an overview of the current status and performance of your NetGain EM. Basic information includes:
Sites:number of monitored sites Devices:number of monitored devices Monitors:number of monitoring checks being performed Critical Status: total number of critical issues from the sites Major Status:total number of major issues from the sites Minor Status :total number of minor issues from the sites Warning Status:total number of warnings issues from the sites Alarms Summary:severity and count of various issues Logs volume:a chart display the volume of application and system logs over time Share dashboard/panel Click onto to share dashboard/panel
Once clicked,the page will navigate to the sharing options page.
...
Adding New BizViews Adding Services to BizView Adding a New Service Dependency `}),e.add({id:43,href:"/modules/home/dashboards/",title:"Dashboards",content:`Dashboards are used to provide the status of the infrastructure at a view. Dashboards can be added and configured to what the user needs.
...
`}),e.add({id:45,href:"/modules/alerts/notificationemail/",title:"Notification Email",content:`Email is used for configuring and testing emails alerts for the system, allowing users to receive notifications via email.
...
`}),e.add({id:46,href:"/administrative/admin/accounts/accesscontrol/",title:"Access Control",content:`This module defines the access control rules to only grant the access for the authorized users.
...
`}),e.add({id:47,href:"/administrative/admin/accounts/",title:"Accounts",content:`The administrator will create and manage accounts for the user.
...
Creation and management of Roles Creation and management of Users Access Control rules Change of My Profile `}),e.add({id:48,href:"/modules/alerts/actiontriggers/",title:"Action Triggers",content:`Action Trigger alarms can be configured to be executed when an alarm is generated. The auto-actions that match the filtering criteria will be invoked for each alarm generated.
`}),e.add({id:49,href:"/modules/siem/log_analytics/reports/adhoc/",title:"Ad Hoc",content:`Click on &lt;AdHoc&gt; to access the AdHoc reports.
...
Edit – To edit report
`}),e.add({id:50,href:"/modules/siem/reports/adhoc/",title:"Ad Hoc",content:`Click on &lt;AdHoc&gt; to access the AdHoc reports.
By &lt;Default&gt; AdHoc will contain report template according to the following topics.
Windows logon failed Authentications failures Network Conversations Windows Account Changes Click on each report to create the report. The report will show in the report area.
AdHoc report sub menu + Add new report – To add a new report
- Select time range of report.
...
Download – To download report
Edit – To edit report
`}),e.add({id:51,href:"/modules/home/dashboards/adddashboard/",title:"Adding a New Dashboard",content:`You can add a new dashboard to the dashboard page and customize it to your requirements.
...
`}),e.add({id:52,href:"/modules/home/business_views/servicedependency/",title:"Adding a New Service Dependency",content:`To add a service dependency under the service, select the service and click on &lt;Edit&gt;. Then click on to add a service dependency
...
`}),e.add({id:53,href:"/modules/cmdb/addingdevices/",title:"Adding Devices to cmdb",content:`Click on &lt;Devices&gt; at the top menu of &lt;Network Config&gt;, then click on &lt;Discover&gt; to find network devices.
...
`}),e.add({id:54,href:"/modules/home/business_views/addbizview/",title:"Adding New BizViews",content:`Click on &lt;Metrics&gt; at the Left Navigation Bar, then click on &lt;Metrics&gt; at the Top Navigation Bar. Click on &lt;Objects&gt; in the left side of the top menu, then click on &lt;Business view&gt;, and click on &lt;Edit&gt;.
Click on and input the Business name. Click &lt;Add&gt;. to add a new BizView.
`}),e.add({id:55,href:"/modules/home/business_views/addservice/",title:"Adding Services to BizView",content:`To add a service under a business, select the business under &lt;Business view&gt; and click on &lt;Edit&gt;. Then click on to add a service.
Input the name of the new service then click .
`}),e.add({id:56,href:"/modules/home/dashboards/addwidget/",title:"Adding Widgets to the Dashboard",content:`Click on the &lt;Add widget &gt; button at the center of the screen to add widgets.
...
Logs: APM: NetFlow: Monitor (Metrics): Alarm (Alerts): Bizviews: Maps: Tools: `}),e.add({id:57,href:"/administrative/admin/",title:"Admin",content:`The Admin Module is designed to empower administrators with comprehensive management capabilities for the NetGain Systems Infrastructure Monitoring system. Accessible exclusively to users with the admin role, this module offers a robust set of tools to effectively oversee and configure your monitoring environment.
...
Account Jobs System ElasticSearch Logs `}),e.add({id:58,href:"/modules/apm/agent/",title:"Agent",content:`Agents are software components used to collect data for monitoring and management purposes.
...
`}),e.add({id:59,href:"/administrative/tools/agentdownload/",title:"Agent Download",content:`Agent Download is used to guide users through the process of installing the NetGain Agent on different operating systems, providing instructions and necessary download links.
...
`}),e.add({id:64,href:"/modules/alerts/alertcorrelation/",title:"Alert Correlation",content:`Root Cause rule defines the relation of a root cause object’s alarm to the dependent object’s alarm in a time window. We can define such rules for well-known dependencies, such as a web-server and a web-site.
...
`}),e.add({id:65,href:"/modules/alerts/alertnotifications/",title:"Alert Notifications",content:`Notifications will be sent to relevant persons in the form of e-mails, SMS, 3rd party app messaging, sound or pop-up windows when a fault or service level violation occurs in the managed environment.
`}),e.add({id:66,href:"/modules/alerts/alertpolicies/",title:"Alert Policies",content:`This lets you create different policies for different alerts.
`}),e.add({id:67,href:"/modules/apm/apm/",title:"APM",content:`In this APM tab is the main tab where you can get an overview of the services health status as well into the details to check each application transaction status and parameters.
Dashboard Services Synthetic Monitoring `}),e.add({id:68,href:"/modules/netflow/settings_maintenance/applications/",title:"Applications",content:`By default, NetGain Systems will generate all known applications. User can create new applications based on TCP/UDP protocol. Click &lt;+Create&gt;.
...
`}),e.add({id:69,href:"/modules/siem/log_analytics/reports/applications/",title:"Applications",content:`Click on &lt;Applications&gt; to access the applications report.
...
`}),e.add({id:70,href:"/modules/siem/reports/applications/",title:"Applications",content:`Click on &lt;Applications&gt; to access the applications report.
Application will contain the report templates according to the following topics:
DHCP Windows Printer Windows Terminal Windows In &lt;DHCP Windows&gt; there are 23 report templates for Windows ready to use.
In &lt;Printer Windows&gt; there are 11 report templates ready to use.
In &lt;Terminal Windows&gt; there are 15 report templates ready to use.
`}),e.add({id:71,href:"/modules/siem/log_analytics/archive/",title:"Archive",content:`NetGain Systems SIEM support archive of log file to external storage/NAS storage. The benefits are to keep logs both in live and archive at same time for better data recovery and at the same time allow user to easily restore the logs when needed.
...
`}),e.add({id:72,href:"/modules/metrics/metrics/blackoutschedule/",title:"Blackout Schedule",content:`User will be able to create their own outage plan to reduce incoming false alerts due to scheduled downtime.
...
`}),e.add({id:73,href:"/modules/cmdb/bulkoperations/",title:"Bulk Operations",content:`This lets you perform operations such as to backup or restore the config on many devices at a time. Click on &lt;Devices&gt; at the top menu of &lt;Network Config&gt;, then select the devices for bulk operations.
...
`}),e.add({id:74,href:"/categories/",title:"Categories",content:""}),e.add({id:75,href:"/modules/siem/log_analytics/reports/compliance/ccpa/",title:"CCPA",content:`In &lt;CCPA&gt; there is 1 report template.
Section 1798.150(a)
Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changers Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads `}),e.add({id:76,href:"/modules/siem/reports/compliance/ccpa/",title:"CCPA",content:`In &lt;CCPA&gt; there is 1 report template.
Section 1798.150(a)
Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changers Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads `}),e.add({id:77,href:"/modules/home/dashboards/clonedashboard/",title:"Cloning a Dashboard",content:`Click on the button at the corner to clone the current dashboard.
Give a name for the cloned dashboard
Click on to clone the dashboard
`}),e.add({id:78,href:"/administrative/admin/elasticsearch/clusternodes/",title:"Cluster Nodes",content:`This module provides overview of the Elasticsearch nodes health condition.
...
`}),e.add({id:79,href:"/modules/siem/log_analytics/reports/compliance/coco/",title:"COCO",content:`In &lt;COCO&gt; there are 5 report templates.
...
`}),e.add({id:82,href:"/modules/siem/reports/common/",title:"Common",content:`Click on &lt;Common&gt; to access the report. Common will contain report template of Windows.
There are 282 report templates for Windows ready to use.
Click on the desired report format. The system will generate a report. As an example of the report as shown in the figure below, chosen to generate a &lt;AD Backup Error&gt; report.
The information shown in the report includes:
Report Name – Show report name.
...
Refresh – Refresh report.
Close - Close report page and back to previous page
`}),e.add({id:83,href:"/modules/siem/log_analytics/reports/compliance/",title:"Compliance",content:`Click on &lt;Compliance&gt; to access the compliance report.
...
`}),e.add({id:86,href:"/installation/emedge/emedge-agent/config_checks/",title:"Configure checks",content:" Files Directories Processes Scripts Windows Services "}),e.add({id:87,href:"/administrative/admin/jobs/autodiscovery/",title:"Create Auto Discovery Task",content:`Select the Schedule task Type as &lt;Auto discovery&gt; .
...
`}),e.add({id:88,href:"/administrative/admin/jobs/deviceshutdown/",title:"Create Device Shutdown Task",content:`Select the Schedule task Type as &lt;Device shutdown task&gt;
...
Once finished, click on &lt;Save&gt; to save the changes.
`}),e.add({id:89,href:"/modules/alerts/currentalerts/",title:"Current Alerts",content:`Device metrics that exceed set thresholds will generate an alert. Current alerts are alerts that have not been acknowledged.
...
`}),e.add({id:90,href:"/modules/metrics/customize/customizedatabase/",title:"Customize Database",content:`This function lets the user query different database methods like MSSQL, Oracle, DB2, Sybase.
...
`}),e.add({id:92,href:"/modules/metrics/customize/powershell-script/",title:"Customize Powershell Script",content:`This function lets the user customize different scripting methods Powershell scritps.
...
`}),e.add({id:93,href:"/modules/metrics/customize/customizescript/",title:"Customize Script",content:`This function lets the user customize different scripting methods like Unix/Linux Shell, Visual Basic and Java Script.
To create Script monitor, Click &lt;Metrics&gt; on the Left Navigation Bar then &lt;Customize&gt; then &lt;Script&gt;. Click &lt;+Add&gt;.
Enter the details below and click &lt;save&gt;.
Label - Appropriate label for the monitor
Argument Count – Number of arguments
...
Add monitor Value – To add different parameters based from the script
`}),e.add({id:94,href:"/modules/metrics/customize/customizesnmp/",title:"Customize SNMP",content:`This feature is used to monitor a specific SNMP OID.
...
`}),e.add({id:95,href:"/modules/metrics/customize/",title:"Customize SNMP, Scripts, Database and Dynamic Thresholds",content:" Customize SNMP Customize Script Customize Powershell-Script Customize Database Customize Dynamic Threshold "}),e.add({id:96,href:"/modules/siem/log_analytics/reports/compliance/cyberessentials/",title:"CYBER ESSENTIALS",content:`In &lt;CYBER ESSENTIALS&gt; there are 5 report templates.
...
Windows : Audit Policy Changed Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : trusted Domain Modified Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Rights Assigned Windows : User Rights Removed Windows : Windows Individual User Action `}),e.add({id:97,href:"/modules/siem/reports/compliance/cyberessentials/",title:"CYBER ESSENTIALS",content:`In &lt;CYBER ESSENTIALS&gt; there are 5 report templates.
Boundary firewall and internet gateways:
Windows : Firewall Flood Attack Windows : Firewall Internet Protocol half-scan attack Windows : Firewall Ping of Death Attack Windows : Firewall SYN Attack Windows : Firewall Spoof Attack Malware Protection
Windows : Defender Malware Detection Windows : Defender Real Time Protection Detection Windows : DOS Attacks Windows : Downgrade Attacks Windows : Event Logging Service Shutdown Windows : Event Logs Cleared Windows : Exe or DLL File Allowed to Run Windows : Exe or DLL File Not Allowed to Run Windows : Exe or DLL Files Not Allowed to Run due to Enforced rules Windows : IP Conflicts Windows : MSI Script File Allowed to Run Windows : MSI Script Files Not Allowed to Run due to Enforced rules Windows : Removeable Disk Creates Windows : Removeable Disk Deletes Windows : Removeable Disk Failed Creates Windows : Removeable Disk Failed Deletes Windows : Removeable Disk Failed Modifications Windows : Removeable Disk Failed Reads Windows : Removeable Disk Modifications Windows : Removeable Disk Reads Windows : Replay Attack Windows : Security Logs Cleared Windows : Software Installed Windows : Software Restricted to Access Program Windows : Terminal Server Attacks Windows : Threat Detections by Mcafee Windows : Threats Detection by Microsoft Anti-malware Windows : Threats Detection by Norton AntiVirus Windows : Threats Detection by Sophos Anti-Virus Windows : Threats Detections by ESET Endpoint Antivirus Windows : User Account Locked Out Error Patch Management: Windows : AD Backup Error
Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Failed hotpatching Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Install Windows : Service Failed Windows : Service Started Windows : Service Stopped Windows : Software Install Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown Secure Configuration:
Windows : Audit Policy Changed Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : Group Created Windows : Group Deleted Windows : Group Modified Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : UnSuccessful Post Authentication Windows : UnSuccessful Pre Authentication Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified Windows : User Right Assigned Windows : User Right Removed Windows : Windows Firewall Group Policy Changes Windows : Windows Firewall Rule Added Windows : Windows Firewall Rule Deleted Windows : Windows Firewall Rule Modified Windows : Windows Firewall Setting Changed Windows : Windows Firewall Setting Restored Windows : Windows Individual User Action User Access Control:
Windows : Audit Policy Changed Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : trusted Domain Modified Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Rights Assigned Windows : User Rights Removed Windows : Windows Individual User Action `}),e.add({id:98,href:"/modules/apm/apm/dashboard/",title:"Dashboard",content:`The dashboard shows the overall information includes below:
...
`}),e.add({id:99,href:"/administrative/admin/logs/debuglog/",title:"Debug Logs",content:`Click on &lt;Debug logs&gt; in the Top navigation bar.
Turn on the debug log for debugging purpose only, choose the level of the logs to enable debugging.
`}),e.add({id:100,href:"/modules/home/dashboards/deletedashboard/",title:"Deleting Dashboard",content:`Click on the button at the corner to delete dashboard.
`}),e.add({id:101,href:"/modules/cmdb/deviceconfiguration/",title:"Device Configuration Management",content:`Click on &lt;Devices&gt; at the top menu of &lt;Network Config&gt;, then click on the name of the device whose configuration you want to manage.
...
`}),e.add({id:104,href:"/modules/metrics/discovering/",title:"Discovering / Adding Devices to be Monitored",content:` Discover Device To discover or add devices to be monitored, click on &lt;Metrics&gt; in the Left Navigation Bar, then &lt;Discover&gt; in the Top Navigation Bar.
...
Discover Device: Discovers devices and monitors each device using IP address range and specific IP addresses `}),e.add({id:105,href:"/modules/metrics/editsettings/",title:"Editing Monitoring Settings",content:`Click on &lt;Metrics&gt; in the Left Navigation Bar, then &lt;Metrics&gt; in the Top Navigation Bar. Then click on &lt;Objects&gt; in the top menu, and click on &lt;Device view&gt;.
...
`}),e.add({id:106,href:"/modules/home/dashboards/editwidget/",title:"Editing Widgets",content:`Click on the button at the center to edit the widget.
Click on to save changes
`}),e.add({id:107,href:"/administrative/admin/elasticsearch/",title:"ElasticSearch",content:`This module provides the information of Elasticsearch, and also provide the functions to configure data retention, backup and restore, indices status.
Click on &lt;Elasticsearch&gt; in the navigation bar.
Maintenance Indices Shards Cluster Nodes `}),e.add({id:108,href:"/administrative/emedge_sites/",title:"Emedge Sites",content:`Sites are used for monitoring and managing distinct environments or deployments within the organization.
...
Information includes:
Infra Monitoring (Nodes): Bar chart showing the number of nodes being monitored over time Logs: Bar chart showing the daily volume of logs in megabytes NetFlow: Bar chart showing the daily volume of NetFlow data in megabytes Network Configuration Management: Bar chart showing the number of nodes involved in network configuration management Application Performance Monitoring: Bar chart showing the daily transaction spans `}),e.add({id:109,href:"/modules/siem/log_analytics/reports/compliance/ferpa/",title:"FERPA",content:`In &lt;FERPA&gt; there is 1 report template.
Section 99.31 (a)(1)(ii)
Windows : File Created Windows : Files Deleted Windows : Files Modified Windows : File Permission Changes Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads `}),e.add({id:110,href:"/modules/siem/reports/compliance/ferpa/",title:"FERPA",content:`In &lt;FERPA&gt; there is 1 report template.
Section 99.31 (a)(1)(ii)
Windows : File Created Windows : Files Deleted Windows : Files Modified Windows : File Permission Changes Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads `}),e.add({id:111,href:"/modules/siem/log_analytics/filters/",title:"Filters",content:`User can use filters function to exclude processing certain logs.
Example: To exclude logs that contain “session event 1656” .
After setting filter, user can test the rule using &lt;Test filters&gt;.
`}),e.add({id:112,href:"/modules/siem/log_analytics/reports/compliance/fisma/",title:"FISMA",content:`In &lt;FISMA&gt; there are 10 report templates.
Access Control (AC)
...
Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected `}),e.add({id:113,href:"/modules/siem/reports/compliance/fisma/",title:"FISMA",content:`In &lt;FISMA&gt; there are 10 report templates.
Access Control (AC)
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logons Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon Access Control (AC)
Successful/Unsuccessful User Logons/Logoffs Audit and Accountability (AU)
Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes Audit and Accountability (AU) – Object Access
...
Windows : Failed Windows backup Windows : Failed Windows restores Windows : Successful Windows restores Windows Successful Windows backup Identification and Authentication (IA)
Windows : Windows individual User Action Information System Monitoring(SI-4)
Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected `}),e.add({id:114,href:"/modules/siem/log_analytics/reports/compliance/gdpr/",title:"GDPR",content:`In &lt;GDPR&gt; there are 4 report templates.
GDPR ARTICLE 32 (1B)
...
Windows : Audit Policy Changed Windows : Failed Network Logons Windows : Failed User Logons Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : User Rights Assigned Windows : User Right Removed Windows : Windows individual User Action `}),e.add({id:115,href:"/modules/siem/reports/compliance/gdpr/",title:"GDPR",content:`In &lt;GDPR&gt; there are 4 report templates.
GDPR ARTICLE 32 (1B)
Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : Group Created Windows : Group Deleted Windows : Group Modified Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified GDPR ARTICLE 32 (1D)
Windows : File Created Windows : File Deleted Windows : File modified Windows : File Permission Changes GDPR ARTICLE 5 (1B)
Windows : Audit Policy Changed Windows : Failed Network Logons Windows : Failed User Logons Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authenticaation Windows : User Logoff Windows : User Logon Windows : User Rights Assigned Windows : User Right Removed Windows : Windows individual User Action GDPR ARTICLE 5 (1F)
Windows : Audit Policy Changed Windows : Failed Network Logons Windows : Failed User Logons Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : User Rights Assigned Windows : User Right Removed Windows : Windows individual User Action `}),e.add({id:116,href:"/modules/home/maps/geomap/",title:"Geographical Map",content:`Google Map provides a geographical view of the devices in the IT infrastructure.
...
`}),e.add({id:117,href:"/modules/siem/log_analytics/reports/compliance/glba/",title:"GLBA",content:`In &lt;GLBA&gt; there are 2 report templates.
Section 501B (1)
...
`}),e.add({id:120,href:"/modules/siem/log_analytics/reports/compliance/gpg/",title:"GPG",content:`In &lt;GPG&gt; there are 5 report templates.
...
Windows : Audit Policy Changed Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Right Assigned Windows : User Rights Removed `}),e.add({id:121,href:"/modules/siem/reports/compliance/gpg/",title:"GPG",content:`In &lt;GPG&gt; there are 5 report templates.
Recording Relating to Network Connections (PMC Rule 6)
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logon Windows : User Logoff Windows : User Logon Recording on Data Backup status (PMC Rule 8)
Windows : Failed Windows Backup Windows : Failed Windows Restores Windows : Successful Windows restore Windows : Successful Windows backup Recording on Internet Workstation, Server or Device Status (PMC Rule 4)
Windows : Registry Created Windows : Registry Deleted Windows : Registry Value Modified Reporting on The Status of The Audit System (PMC Rule 10)
Windows : Defender Malware Detection Windows : Defender Real Time Protection Detection Windows : DoS Attacks Windows : Downgrade Attacks Windows : Event Loggins Service Shutdown Windows : Event Logs Cleared Windows : IP Conficts Windows : Replay Attacks Windows : Security Logs Cleared Windows : Terminal Server Attacks Windows : User Account Locked Out Error Suspicious Internal Networks Activity (PMC Rule 5)
Windows : Audit Policy Changed Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Right Assigned Windows : User Rights Removed `}),e.add({id:122,href:"/modules/siem/log_analytics/reports/compliance/hipaa/",title:"HIPAA",content:`In &lt;HIPAA&gt; there are 6 report templates.
164.308(a)(1)(ii)(D)
...
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon 164.308(a)(6)(ii)
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon 166.308(a)(7)(i)
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Software Installed Windows : Software uninstalled Windows : Software updated Windows : Windows Startup and Windows Shutdown Account Logon Object Access `}),e.add({id:124,href:"/modules/alerts/historyalerts/",title:"History Alerts",content:`This gives an overview of the historical alert notifications based on the category selected. To go to History Alerts, click on &lt;Alerts&gt; on the Left Navigation Bar, then click &lt;Alerts&gt; on the Top Navigation Bar. Click on &lt;History&gt;.
Select the Alert category and the date range of the alerts that you would like to see.
`}),e.add({id:125,href:"/",title:"Home",content:`Welcome to NetGain docs!
...
NetGain Systems (c) 2002-2021. All rights reserved. `}),e.add({id:126,href:"/administrative/admin/elasticsearch/indices/",title:"Indices",content:`All performance data and logs are stored in Elasticsearch on a daily basis as indices.
...
`}),e.add({id:127,href:"/installation/emedge/emedge-agent/installation/",title:"Installation",content:`Supported environments emedge-agent is now supported on most common flavors Linux, Unix, Windows and Apple OSX.
...
Download and unzip latest emedge-agent software package for FreeBSD cd into emedge-agent folder and run emedge-agent config Installation as system service on unix/linux Linux/Unix/OSX: Run /opt/emedge-agent/emedge-agent service install Further details All logs will be saved into emedge-agent.log in the installation directory To stop emedge, simply run emedge-agent stop Note: run emedge-agent -h for more help on other commands `}),e.add({id:128,href:"/installation/emedge/installation/",title:"Installation",content:`To get started, select your platform Installation steps for emedge on Windows Method 1
...
`}),e.add({id:132,href:"/modules/alerts/integrations/",title:"Integrations",content:" Line notifications MSTeams notifications "}),e.add({id:133,href:"/administrative/admin/logs/internallog/",title:"Internal Logs",content:`Click on &lt;Internal logs&gt; in the Top navigation bar.
...
`}),e.add({id:134,href:"/administrative/tools/diagnostic/ipcalculator/",title:"IP Calculator",content:`This tool helps user to calculate IP address.
...
`}),e.add({id:135,href:"/modules/siem/log_analytics/reports/compliance/islp/",title:"ISLP",content:`In &lt;ISLP&gt; there are 8 report templates.
ARTICLE 12
...
Windows : Failed Windows backup Windows : Filed Windows restores Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Successful Windows restores Windows : Successful windows backup ARTICLE 30.4
Windows : Failed Windows backup Windows : Filed Windows restores Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Successful Windows restores Windows : Successful windows backup ARTICLE 30.6
Windows : Audit Policy Changed Windows : Failed Network Logons Windows : Failed User Logons Windows : Failed Windows backup Windows : Filed Windows restores Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Successful Windows restores Windows : Successful windows backup Windows : Terminal Server Disconnected Windows : Terminal Server Rconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : UnSuccessful Post Authentication Windows : UnSuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : User Right Assigned Windows : User Rights Removed Windows : Windows Individual User Action `}),e.add({id:137,href:"/modules/siem/log_analytics/reports/compliance/iso27002013/",title:"ISO 2700 2013",content:`In &lt;ISO 27001 2013&gt; there are 6 report templates.
Control A 12.4.1
...
Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : Group Created Windows : Group Deleted Windows : Group Modified Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified Windows : Windows Individual User Action `}),e.add({id:138,href:"/modules/siem/reports/compliance/iso27002013/",title:"ISO 2700 2013",content:`In &lt;ISO 27001 2013&gt; there are 6 report templates.
Control A 12.4.1
Windows : Audit Policy Changed Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Group Created Windows : Group Deleted Windows : Group Modified Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified Windows : User Right Assigned Windows : User Rights Removed Windows : Windows Individual User Action Control A 12.4.2
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Audit Policy Changed Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : New Service Installed Windows : Software Installed Windows : Software uninstalled Windows : Software updated Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Right Assigned Windows : User Rights Removed Windows : Windows Startup and Windows Shutdown Control A.12.4.3
Windows : Audit Policy Changed Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : Failed Network Logons Windows : Failed User Logons Windows : Group Created Windows : Group Deleted indows : Group Modified Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Rconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified Windows : User Logoff Windows : User Logon Windows : User Right Assigned Windows : User Rights Removed Windows : Windows Individual User Action Control A 9.2.1
...
`}),e.add({id:141,href:"/modules/home/leftnavbar/",title:"Left Navigation Bar",content:`The Left Navigation Bar of the console let users access the different modules of NetGain Systems ITM&amp;SM and is always available throughout the console.
There is a small arrow at the base of the menu to expand the icon names.
Button Description Directs you to the home page of the spog.ai console, where you can access Dashboards, BizView and Maps. Lets users check the status or performance of devices or servers over a period of time. Also, where you can discover or add devices to be monitored. Optional SIEM module which includes Log Analytics and Security Analytics. Optional NetFlow module for network traffic details. Optional APM (Application Performance Management) module for application monitoring and management. Optional CMDB (Configuration management database) module for Asset Management, Network Configuration Management and IP Address Management. Lets you define alarms and how notification messages are to be sent to different IT team members. Built-in tools available on the spog.ai console, including sending pings and SNMP settings. System administrator functions, including adding / changing end-user information/settings. `}),e.add({id:142,href:"/administrative/admin/system/license/",title:"License",content:`License shows the user the details of the NetGain Systems Infrastructure Monitoring license information on enabled modules, used license nodes, total license nodes.
Click on the &lt;License&gt; in the Top Navigation Bar.
The page will show the info:
Serial number Infrastructure Monitoring model License Expiry date Max licensed nodes XXXX (currently used: XX) Max NetFlow Flow Rate XXX flows per sec Add-ons modules eg. Unlimited Ping, Network Config Manager, IP Address Manager, Asset Manager Advance monitors `}),e.add({id:143,href:"/troubleshooting_guides/FAQ/licensing/",title:"Licensing",content:`How to update license Click on Admin, System, License.
Choose the license file and click on to upload the license.
`}),e.add({id:144,href:"/modules/alerts/integrations/line/",title:"LINE notifications",content:`This document is to provide a step by step setup for Line App integration to be setup as the notifications channel
...
`}),e.add({id:145,href:"/modules/home/maps/logmap/",title:"Logical Map",content:`Logical Map allows the user to set various types of backgrounds so that there is a useful representation of the enterprise network and systems.
...
`}),e.add({id:146,href:"/administrative/admin/logs/",title:"Logs",content:`Click on &lt;Logs&gt; in the Top navigation bar.
System Logs Operation Logs Notification Logs Scheduler Logs Internal Logs Debug Logs `}),e.add({id:147,href:"/administrative/admin/elasticsearch/maintenance/",title:"Maintenance",content:`Click on &lt;Maintenance&gt; in the Top navigation bar.
...
Upload local backup: to upload a local backup copy to the system. Backup now: to do the configuration backup now. Download to local: to download the configuration as a zipped copy to the local drive. Restore: restore the configuration that you selected. Delete: delete the backup copy. `}),e.add({id:148,href:"/modules/netflow/settings_maintenance/maintenance/",title:"Maintenance",content:`The &lsquo;Performance&rsquo; tab displays various flow-related metrics and statistics for the network traffic being monitored and analyzed by the NetFlow system.
Information includes:
...
`}),e.add({id:149,href:"/modules/siem/log_analytics/mapping/",title:"Mapping",content:`NetGain Systems supports some log mapping by default. The common logs follows Common Event Format (CEF), and some other mappings supported by default are Aruba, Cisco, Fortinet logs etc.
...
`}),e.add({id:150,href:"/modules/metrics/metrics/",title:"Metrics Policy",content:`The module allows the user to configure or setup a policy for the devices and monitors. To access Policy module, click &lt;Metrics&gt; on the Left Navigation Bar then &lt;Policy&gt;.
Blackout Schedule SNMP V3 Users Global Rules Rules Audit `}),e.add({id:151,href:"/administrative/tools/diagnostic/mibbrowser/",title:"MIB Browser",content:`This tool helps user to load the devices’ MIBs file to NetGain Systems Infrastructure Monitoring server, to browse OID information and to allow user to know the particular OID of the device.
...
`}),e.add({id:152,href:"/modules/siem/mitreattack/",title:"MITRE ATT&CK",content:`MITRE ATT&amp;CK® is a globally-accessible knowledge base of adversary tactics and techniques based on real-world observations. The ATT&amp;CK knowledge base is used as a foundation for the development of specific threat models and methodologies in the private sector, in government, and in the cybersecurity product and service community.
...
`}),e.add({id:153,href:"/modules/siem/security_analytics/mitreattack/",title:"MITRE ATT&CK",content:`MITRE ATT&amp;CK® is a globally-accessible knowledge base of adversary tactics and techniques based on real-world observations. The ATT&amp;CK knowledge base is used as a foundation for the development of specific threat models and methodologies in the private sector, in government, and in the cybersecurity product and service community.
MITRE ATT&amp;CK is an information tab that allow user to link certain threats to the MITRE techniques ID. User can refer more info from this link: https://attack.mitre.org/
`}),e.add({id:154,href:"/modules/alerts/monitoralerts/",title:"Monitor Alerts",content:`NetGain Systems Infrastructure Monitoring uses the following color codes to distinguish between the different severity of alerts.
Color Severity Color Severity Red Critical Blue Warning Orange Major Green Normal Yellow Minor Gray Disabled `}),e.add({id:155,href:"/modules/metrics/monitor-business/",title:"Monitor by Business Types",content:`Business view is used to monitor the health and status of various business services within the system.
...
`}),e.add({id:156,href:"/modules/metrics/monitor-by-device-types/",title:"Monitor by Device Type",content:`Click on &lt;Metrics&gt; in the Left Navigation Bar, then &lt;Metrics&gt; in the Top Navigation Bar. Then click on &lt;Objects&gt; in the top menu, and click on &lt;Device view&gt;.
...
`}),e.add({id:157,href:"/troubleshooting_guides/FAQ/monitoringdiscovery/",title:"Monitoring & Discovery",content:`In Constructions
`}),e.add({id:158,href:"/modules/metrics/monitoralerts/",title:"Monitoring by Alert Types",content:`You can select the severity of the alerts at the top of the table to see those see the status of the devices that have the selected alert type. You can also make changes to the alert threshold settings.
...
`}),e.add({id:159,href:"/modules/alerts/integrations/msteams/",title:"MSTeams notifications",content:`This document is to provide a step by step setup for MS Teams integration to be setup as the notifications channel.
...
`}),e.add({id:160,href:"/administrative/admin/accounts/myprofile/",title:"My Profile",content:`This module allows you to change your current login user profile.
...
`}),e.add({id:161,href:"/modules/siem/log_analytics/reports/compliance/nerc/",title:"NERC",content:`In &lt;NERC&gt; there are 10 report templates.
CIP 005-6 R1.3
...
Windows : Windows Firewall Group Policy Changes Windows : Windows Firewall Rule Added Windows : Windows Firewall Rule Deleted Windows : Windows Firewall Rule Modified Windows : Windows Firewall Setting Changed Windows : Windows Firewall Setting Restored Windows : Windows Individual User Action CIP 009-6 R1.4
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Failed Windows backup Windows : Filed Windows restores Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Service Failed Windows : Service Started Windows : Service Stopped Windows : Software Installed Windows : Software Uninstalled Windows : Software Updated Windows : Successful Windows restores Windows : Successful windows backup Windows : Windows Startup and Windows Shutdown CIP 010-2 R1.1
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Exe or DLL File Allowed to Run Windows : Exe or DLL File Not Allowed to Run Windows : Exe or DLL Files Not Allowed to Run due to Enforced rules Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : MSI Script File Allowed to Run Windows : MSI Script Files Not Allowed to Run due to Enforced rules Windows : New Service Installed Windows : Service Failed Windows : Service Started Windows : Service Stopped Windows : Software Installed Windows : Software Restricted to Access Program Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown `}),e.add({id:163,href:"/modules/siem/log_analytics/reports/compliance/nist/",title:"NIST",content:`In &lt;NIST&gt; there are 2 report templates.
Account Logon
Data Security (PR.DS)
...
Windows : Threat Detections by Mcafee Windows : Threats Detection by Microsoft Antimalware Windows : Threats Detection by Norton AntiVirus Windows : Threats Detection by Sophos Anti-Virus Windows : Threats Detections by ESET Endpoint Antivirus `}),e.add({id:164,href:"/modules/siem/reports/compliance/nist/",title:"NIST",content:`In &lt;NIST&gt; there are 2 report templates.
Account Logon
Data Security (PR.DS)
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Rconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected Object Access Risk Assessment (ID.RA)
Windows : Threat Detections by Mcafee Windows : Threats Detection by Microsoft Antimalware Windows : Threats Detection by Norton AntiVirus Windows : Threats Detection by Sophos Anti-Virus Windows : Threats Detections by ESET Endpoint Antivirus `}),e.add({id:165,href:"/administrative/admin/logs/notificationlog/",title:"Notification Logs",content:`Click on &lt;Notification logs&gt; in the navigation bar.
It provides the history detailed records that how many notification messages been sent.
`}),e.add({id:166,href:"/modules/alerts/notificationrules/",title:"Notification Rules",content:`You can create Notification Rules to send SMS, email, 3rd party app messages, or audio and pop-up Windows alerts when your devices reach preset critical threshold levels. To create a Notification Rule, click on &lt;Alerts&gt; on the Left Navigation Bar, then click on &lt;Notifications&gt; on the Top Navigation Bar. Click on &lt;Rules&gt;, then &lt;+Create&gt;.
...
`}),e.add({id:167,href:"/modules/siem/log_analytics/reports/compliance/nrc/",title:"NRC",content:`In &lt;NRC&gt; there are 18 report templates.
ACT B.1.11
...
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon `}),e.add({id:168,href:"/modules/siem/reports/compliance/nrc/",title:"NRC",content:`In &lt;NRC&gt; there are 18 report templates.
ACT B.1.11
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Rconnected Windows : UnSuccessful Post Authentication Windows : UnSuccessful Pre Authentication Windows : User Logoff Windows : User Logon ACT B.1.15
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Rconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon ACT B.1.17
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Software Installed Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected ACT B.1.19
...
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Software Installed Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown ACT C.3.7
Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes ACT C.4.3
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon `}),e.add({id:169,href:"/administrative/admin/logs/operationlog/",title:"Operation Logs",content:`Click on &lt;Operation logs&gt; in the navigation bar.
It provides NetGain Systems Infrastructure Monitoring user login event, change of account or other Ops related events.
`}),e.add({id:170,href:"/modules/alerts/integrations/opsgenie/",title:"Opsgenie notifications",content:`This document is to provide a step by step setup for Opsgenie App integration to be setup as the notifications channel
Add an API integration Add an API integration for Free plan, Essentials plan and Jira Service Management&rsquo;s Standard plan Setup Opsgenie on NetGain Create Notification Category Create Notification Rules Add an API integration Create Opsgenie account. Go to Settings &gt; Integrations. Run a search and select “API”. On the next screen, enter a name for the integration. Optional: Select a team in Assignee team if you want a specific team to receive alerts from the integration. Learn how alert notifications flow in Opsgenie. Select Continue. The integration is saved at this point. Expand the Steps to configure the integration section and copy the API key for use in your API requests. Select Turn on integration. The rules you create for the integration will work only if you turn on the integration. Note: If you&rsquo;re using Opsgenie&rsquo;s Free or Essentials plan or if you’re using Opsgenie with Jira Service Management&rsquo;s Standard plan, you can add this integration from your team dashboard only. The Integrations page under Settings is not available in your plan.
Add an API integration for Free plan, Essentials plan and Jira Service Management&rsquo;s Standard plan Go to Teams and add your team. Enter team details. Select Integrations on the left navigation and then select Add integration. Search for API. Enter integration name &gt; select team &gt; click continue. Click turn on integration and copy API key. Adding the integration from your team dashboard will make your team the owner of the integration. This means Opsgenie will assign the alerts received through this integration to your team only. Follow the rest of the steps in this section to set up the integration. If the Owner of your Opsgenie account has not verified their email yet, users within your organization will not be able use API integrations.
Setup Opsgenie on NetGain Go to Alerts &gt; Notifications &gt; Integrations &gt; click Opsgenie. Click create. Enter Label and API key. Click Send test message to ensure the API key is valid. Create Notification Category Go to NetGain web GUI &gt; Alerts &gt; Policies &gt; Categories &gt; click Create. Enter the details of the category and click save. Create Notification Rules Go to Alerts &gt; Notifications &gt; Rules &gt; click Create. Enter the details &gt; tick Opsgenie &gt; click Save. `}),e.add({id:171,href:"/modules/alerts/othersettings/",title:"Other Alert Notification Settings",content:`Besides Email and SMS notification, there are other methods to alert users, including 3rd party messaging apps such as Slack, MS Teams, WeChat, Alicloud, LINE and Telegram.
...
`}),e.add({id:172,href:"/modules/siem/log_analytics/reports/compliance/pcidss/",title:"PCI-DSS",content:`In &lt;PCI-DSS&gt; there are 6 report templates.
...
Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes `}),e.add({id:173,href:"/modules/siem/reports/compliance/pcidss/",title:"PCI-DSS",content:`In &lt;PCI-DSS&gt; there are 6 report templates.
PCI-DSS requirements 10.1
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Registry Created Windows : Registry Deleted Windows : Registry Value Modified Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action PCI-DSS requirements 10.2.1
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon PCI-DSS requirements 10.2.2
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action PCI-DSS requirement 10.2.3
Windows : Audit Policy Changed Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Registry Created Windows : Registry Deleted Windows : Registry Value Modified Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Logoff Windows : User Logon Windows : User Right Assigned Windows : User Rights Removed Windows : Windows Firewall Group Policy Changes Windows : Windows Firewall Rule Added Windows : Windows Firewall Rule Deleted Windows : Windows Firewall Rule Modified Windows : Windows Firewall Setting Changed Windows : Windows Firewall Setting Restored PCI-DSS requirements 10.2.6
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Software Installed Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown PCI-DSS requirements 10.2.7
Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission Changes `}),e.add({id:174,href:"/modules/siem/log_analytics/reports/compliance/pdpa/",title:"PDPA",content:`In &lt;PDPA&gt; there are 2 report templates.
RULE VI Section 25
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected RULE VII Section 30
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission ChangesWindows : Network Logoffs Windows : Network Logon Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : UnSuccessful Post Authentication Windows : UnSuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected `}),e.add({id:175,href:"/modules/siem/reports/compliance/pdpa/",title:"PDPA",content:`In &lt;PDPA&gt; there are 2 report templates.
RULE VI Section 25
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Crated Windows : File Deleted Windows : File Modified Windows : File Permission Changes Windows : Network Logoffs Windows : Network Logon Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : Unsuccessful Post Authentication Windows : Unsuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected RULE VII Section 30
Windows : Failed Network Logons Windows : Failed User Logons Windows : File Created Windows : File Deleted Windows : File Modified Windows : File Permission ChangesWindows : Network Logoffs Windows : Network Logon Windows : Removable Disk Creates Windows : Removable Disk Deletes Windows : Removable Disk Failed Creates Windows : Removable Disk Failed Deletes Windows : Removable Disk Failed Modifications Windows : Removable Disk Failed Reads Windows : Removable Disk Modifications Windows : Removable Disk Reads Windows : Successful Post Authentication Windows : Successful Pre Authentication Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : UnSuccessful Post Authentication Windows : UnSuccessful Pre Authentication Windows : User Logoff Windows : User Logon Windows : Windows Individual User Action Windows : Wireless Network Authentications Windows : Wireless Network Connected Windows : Wireless Network Disconnected `}),e.add({id:176,href:"/administrative/tools/diagnostic/ping/",title:"Ping",content:`This tool helps user to verify the connectivity between NetGain Systems Infrastructure Monitoring server and the destination devices.
...
`}),e.add({id:177,href:"/troubleshooting_guides/FAQ/reporting/",title:"Report",content:`In Construction
`}),e.add({id:178,href:"/modules/apm/reports/",title:"Reports",content:`The report module is used to generate ad-hoc reports. This module supports:
...
Once done, click on ‘Save’.
`}),e.add({id:179,href:"/modules/netflow/reports/",title:"Reports",content:`You can create different NetFlow reports based on different search expressions.
...
`}),e.add({id:180,href:"/modules/siem/log_analytics/reports/",title:"Reports",content:`To access the SIEM Reports function, click on the &lt;SIEM&gt; icon at the Left Navigation Bar then click on &lt;Reports&gt; at the Top Navigation Bar.
There are 4 sub-menus in the Reports function:
Common Applications Compliance Ad Hoc `}),e.add({id:181,href:"/modules/siem/reports/",title:"Reports",content:`To access the SIEM Reports function, click on the &lt;SIEM&gt; icon at the Left Navigation Bar then click on &lt;Reports&gt; at the Top Navigation Bar.
There are 4 sub-menus in the Reports function:
Common Applications Compliance Ad Hoc `}),e.add({id:182,href:"/administrative/admin/system/restapis/",title:"Rest APIs",content:`This module describes the REST API and resources provided by NetGain Systems Infrastructure Monitoring.
Click on &lt;Rest APIs&gt; in the navigation bar.
`}),e.add({id:183,href:"/administrative/admin/accounts/roles/",title:"Roles",content:`The Administrator can control the newly created user’s access right through Roles. To create new role for the End-User. Click on &lt;+Create&gt; in the page.
...
`}),e.add({id:184,href:"/modules/metrics/metrics/rulesaudit/",title:"Rules Audit",content:`User will be able to determine the standardization of the threshold parameters. It lets the user merge the old settings to new settings to make it standard.
...
`}),e.add({id:185,href:"/modules/home/dashboards/savedashboard/",title:"Saving Dashboard",content:`Click on the button at the corner to save the dashboard.
`}),e.add({id:186,href:"/administrative/admin/logs/schedulerlog/",title:"Scheduler Logs",content:`Click on &lt;Scheduler logs&gt; in the Top navigation bar.
It shows the scheduler running history logs:
`}),e.add({id:187,href:"/modules/cmdb/schedulingbackup/",title:"Scheduling Configuration Backup",content:`This lets you schedule the backup of your Network Configuration. Click on &lt;Schedules&gt; at the top menu of &lt;Network Config&gt;.
...
`}),e.add({id:188,href:"/modules/siem/log_analytics/schema/",title:"Schema",content:`This is information tab about Elastic Common Schema. User can get more from the following link: https://www.elastic.co/guide/en/ecs/current/index.html
`}),e.add({id:189,href:"/modules/siem/log_analytics/search/",title:"Search",content:`SIEM search is for the user to do simple search through keywords or to do advance query string search. Example simple search:
...
`}),e.add({id:190,href:"/modules/siem/search/",title:"Search",content:`The search function here is like the search bar on Log Analytics. User can do simple search or advance query string on the logs that recorded threat. &lt;Help&gt; can be used to learn more about the query syntax.
User can select the time range and look at the number of threat logs on different time range.
`}),e.add({id:191,href:"/modules/siem/security_analytics/search/",title:"Search",content:`The search function here is like the search bar on Log Analytics. User can do simple search or advance query string on the logs that recorded threat. &lt;Help&gt; can be used to learn more about the query syntax.
User can select the time range and look at the number of threat logs on different time range.
`}),e.add({id:192,href:"/troubleshooting_guides/FAQ/security/",title:"Security",content:`How to request CSR generation Please change the following details according to customer requirements.
...
Event.outcome Host.ip Transaction.duration.us Source.ip User OS Transaction type User browser `}),e.add({id:194,href:"/installation/cloud_vista/setting-up/",title:"Setting up",content:"Getting up and running Signup for at CVS account at https://portal.netgain-systems.com/signup Login to your portal at https://portal.netgain-systems.com/login to create your CVS instance Install and setup emedge at local site(s) by following steps in this page Optionally install emedge-agent software on your server hosts that you would like to manage, with these steps Once done, monitoring metrics, logs and traffic data will now be automatically streamed to the assigned CVS instance "}),e.add({id:195,href:"/modules/netflow/settings_maintenance/",title:"Settings and Maintenance",content:`To navigate to NetFlow settings, click &lt;NetFlow&gt; on the Left navigation bar then &lt;Settings&gt;.
Applications Maintenance `}),e.add({id:196,href:"/administrative/admin/elasticsearch/shards/",title:"Shards",content:`Click on &lt;Shards&gt; in the Top navigation bar.
...
`}),e.add({id:197,href:"/troubleshooting_guides/FAQ/snmpinstall/",title:"SNMP Installation & Configuration",content:`How to discover monitors through SNMPV3 Click on Metrics, Discover.
...
`}),e.add({id:198,href:"/modules/metrics/metrics/snmpv3users/",title:"SNMP V3 Users",content:`The SNMP v3 user credentials can be set here.
Click &lt;Metrics&gt; on the Left navigation bar then &lt;Policy&gt;. Under &lt;SNMP v3 Users&gt; click &lt;Add new user&gt;.
`}),e.add({id:199,href:"/administrative/tools/diagnostic/snmpwalk/",title:"SNMPWALK",content:`This tool helps user to test the SNMP connectivity between NetGain Systems EM server to the destination devices and get the SNMP.
...
`}),e.add({id:200,href:"/modules/siem/log_analytics/sources/",title:"Sources",content:`This section enables the user to add logs into the system.
...
`}),e.add({id:201,href:"/modules/siem/log_analytics/reports/compliance/sox/",title:"SOX",content:`In &lt;SOX&gt; there are 7 report templates.
SEC 302(a)(4)(A)
...
Windows : Computer Account Created Windows : Computer Account Deleted Windows : Computer Account Modified Windows : Group Created Windows : Group Deleted Windows : Group Modified Windows : User Account Created Windows : User Account Deleted Windows : User Account Locked Outs Windows : User Account Modified `}),e.add({id:202,href:"/modules/siem/reports/compliance/sox/",title:"SOX",content:`In &lt;SOX&gt; there are 7 report templates.
SEC 302(a)(4)(A)
Windows : AD Backup Error Windows : Audit Logs Cleared Windows : Error in EventLog Service Windows : Event log automatic backup Windows : Falied hotpatcing Windows : Failed software installations Windows : Failed software installations due to privilege mismatches Windows : New Service Installed Windows : Process Accessed Windows : Process Created Windows : Process Duplicated Windows : Process Terminated Windows : Software Install Windows : Software Uninstalled Windows : Software Updated Windows : Windows Startup and Windows Shutdown SEC 302(a)(4)(b)
Windows : Audit Policy Changed Windows : Trusted Domain Created Windows : Trusted Domain Deleted Windows : Trusted Domain Modified Windows : User Right Assigned Windows : User Rights Removed SEC 302(a)(4)(C)
Windows : Failed Network Logons Windows : Failed User Logons Windows : Network Logoffs Windows : Network Logon Windows : Terminal Server Disconnected Windows : Terminal Server Reconnected Windows : User Logoff Windows : User Logon SEC 302(a)(4)(D)
...
`}),e.add({id:205,href:"/administrative/admin/system/",title:"System",content:`System page allows End-User to see the overview of logs on the NetGain Systems Infrastructure Monitoring systems, system diagnostics to check on the system for error and system uptime.
Status License Rest APIs `}),e.add({id:206,href:"/administrative/admin/logs/systemlog/",title:"System Logs",content:`The End-User can quickly download Infrastructure Monitoring system related logs such as System Log, Operation Log, Vista logs, Notifications log, Scheduler log, Internal log, and Debug log as text files or Excel file for analysis.
...
`}),e.add({id:207,href:"/tags/",title:"Tags",content:""}),e.add({id:208,href:"/modules/siem/security_analytics/threatdetection/",title:"Threat Detection",content:`By default, NetGain Systems support around 500 detection rules. Detection rules are done through the query string.
...
`}),e.add({id:209,href:"/modules/siem/threatdetection/",title:"Threat Detection",content:`By default, NetGain Systems support around 500 detection rules. Detection rules are done through the query string.
For default rule, user can &lt;Disable&gt;, &lt;Enable&gt; and &lt;Edit&gt; the tags or description (but not query expression) in the edit.
To add new rule, user can add in &lt;Add new rule&gt;. User just need to specific the name, tags, description accordingly.
User can define the &lt;interval&gt; on how frequent for SIEM to run through this rule. For &lt;search window&gt; under logic type query, user can define the historical search window.
NetGain Systems supports 3 types of logic type: Python(sandbox), Query and Workflow to create new rules.
Python: Python API documentation is available in the GUI Query: Using search query or query expression. Workflow: Flow chart type logic. The workflows can be created in the &lt;AI Workflows&gt; Threat Intelligence &amp; Lookup Tables Threat Intelligence is where NetGain Systems gather the flagged IP or URL from establish sources and compare against the logs. User can choose to enable or disable particular sources.
For lookup tables, user can use it to create table to map key to a value. The key can be used in python script to run certain rules.
`}),e.add({id:210,href:"/modules/siem/security_analytics/threatevents/",title:"Threat Events",content:`In Threat Events, user can search for the threat events based on their threat ID or threat message. Clicks on the events will bring up all the related log that linked to this threat.
User can also select the different time range to check for the events that happened.
`}),e.add({id:211,href:"/modules/siem/threatevents/",title:"Threat Events",content:`In Threat Events, user can search for the threat events based on their threat ID or threat message. Clicks on the events will bring up all the related log that linked to this threat.
User can also select the different time range to check for the events that happened.
`}),e.add({id:212,href:"/modules/metrics/thresholdsettings/",title:"Threshold Settings",content:`Click on &lt;Metrics&gt; in the Left Navigation Bar, then &lt;Metrics&gt; in the Top Navigation Bar. Then click on &lt;Objects&gt; in the top menu, and click on &lt;Device view&gt;. To adjust the threshold settings, click on the device type / device on the right side of the table and drill down till you see the monitoring parameter you would like to change. Click &lt;Edit&gt; to enable the edit mode and click the – Change threshold settings icon to edit the threshold settings.
...
`}),e.add({id:213,href:"/modules/siem/log_analytics/reports/adhoc/addreport/",title:"To add a new report",content:`Click &lt;+Add new report&gt;.
...
Save – Save report
`}),e.add({id:214,href:"/modules/siem/reports/adhoc/addreport/",title:"To add a new report",content:`Click &lt;+Add new report&gt;.
Report title – Enter report title.
Category – Enter report category.
Description – Enter description of report.
Search expression – Enter search expression.
Visibility – Select who can see this report (Owner/All)
Save – Save report
`}),e.add({id:215,href:"/modules/siem/log_analytics/reports/adhoc/autodelivery/",title:"To create auto delivery report setting",content:`
...
Save – Save report
`}),e.add({id:216,href:"/modules/siem/reports/adhoc/autodelivery/",title:"To create auto delivery report setting",content:`
Mail Subject – enter mail subject.
Mail Body – Enter mail body.
Target Address (s) – Enter email address.
Format – Select report file type. (Excel/PDF)
...
Hour of day – Enter number of hour
Save – Save report
`}),e.add({id:217,href:"/modules/siem/log_analytics/reports/adhoc/downloadreport/",title:"To download report",content:`Download Excel – Download report to local computer in Excel file.
Download PDF – Download report to local computer in PDF file.
Download CSV – Download report to local compute in SCV file
`}),e.add({id:218,href:"/modules/siem/reports/adhoc/downloadreport/",title:"To download report",content:`Download Excel – Download report to local computer in Excel file.
Download PDF – Download report to local computer in PDF file.
Download CSV – Download report to local compute in SCV file
`}),e.add({id:219,href:"/modules/siem/log_analytics/reports/adhoc/editreport/",title:"To edit report",content:`
Report title – Enter report title.
Category – Enter report category.
Description – Enter description of report.
Search expression – Enter search expression.
...
Save – Save report
`}),e.add({id:220,href:"/modules/siem/reports/adhoc/editreport/",title:"To edit report",content:`
Report title – Enter report title.
Category – Enter report category.
Description – Enter description of report.
Search expression – Enter search expression.
...
Delete report – Delete report
Save – Save report
`}),e.add({id:221,href:"/administrative/tools/",title:"Tools",content:`NetGain Systems Infrastructure Monitoring provides diagnostic tools for troubleshooting and agents for installation on the servers that needs to be monitored. Click on &lt;Tools&gt; at the left navigation bar.
Diagnostic Agent Download `}),e.add({id:222,href:"/modules/home/topnavbar/",title:"Top Navigation Bar",content:`The Top Navigation Bar displays different functions, depending on the module selected from the Left Navigation Bar. The current selected function will be underlined. For the Home Page, the available functions are:
Dashboards: for default dashboards as well customizable dashboards to display. BizViews: to view the services availability, status and performance from a business or services perspectives. Maps: to view devices status from a logical, physical or geographic view. `}),e.add({id:223,href:"/troubleshooting_guides/FAQ/user_management/",title:"User Management",content:`How to unlock my user account if its locked Admin account is needed in order to unlock user’s account.
...
Change the input to the desired days. `}),e.add({id:224,href:"/administrative/admin/accounts/users/",title:"Users",content:`You can specify different access rights for different roles, for example, specifying that business staff only receive system warnings but not able to perform operations. Different access rights will affect the administrative rights you have, and thus access to features in the Administrator interface.
...
`}),e.add({id:225,href:"/modules/netflow/usingnta/",title:"Using Network Traffic Analytics",content:`The &lsquo;Overview&rsquo; tab displays the general summary of network traffic data.
...
`}),e.add({id:226,href:"/modules/apm/watcher/",title:"Watcher",content:`APM Watcher is used to watch out for specified data within APM. The system generates an alarm when the data contains the specified query string. This is useful for identifying problems quickly.
...
`}),e.add({id:229,href:"/modules/siem/watcher/",title:"Watcher",content:`This function is used to watch out for specified data within incoming data logs. The system sends an alarm when the incoming log data contains a specified key word or phrase. This is useful to detect any known abnormalities or behavior.
To access the Watcher function, click on the &lt;SIEM&gt; icon at the Left Navigation Bar then click on &lt;Watcher&gt; at the Top Navigation Bar if not already at the Reports page.
Add Watcher Rule Click on Watcher &lt; &gt; and when you are in the Watcher menu, click .
Enter the data and rules to define what you want to watch out for and receive an alarm when the conditions are met.
Name – Input name of rule.
...
To view LogWatch alerts When the Watcher rule has been created and enabled, an alert will be generated if the specified key word or phrase is detected.
To see the alert message, click on Alerts icon at the Left Bavigation bar then click on &lt;Alerts&gt; at the Top Navigation Bar than click on &lt;Current&gt; at the sub menu bar then select &lt;LogsWatch Alarms&gt; at the drop down list menu.
`}),e.add({id:230,href:"/modules/home/dashboards/widgetdetails/",title:"Widget Details",content:` Category Widget Name Description Logs Histogram Query string driven. Shows logs histogram by different aggregation methods in the form of bar, line and area graphs.
